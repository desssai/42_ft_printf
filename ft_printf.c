...
/*   Updated: 2021/11/08 02:55:50 by ncarob           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int	ft_print_value(t_flags *flags, va_list *ap)
{
	int	res;
...
void	ft_get_flags(t_flags *flag)
{
	int	i;
...
t_flags	*ft_init_flag(const char **str)
{
	t_flags	*flag;
	char	*flag_end;
...
void	ft_empty_flag(t_flags **flag)
{
	if (!flag)
		return ;
	free((*flag)->body);
